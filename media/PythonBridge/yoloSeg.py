...
class YoloSegEngine:
	...
	def _preprocess(self, bgr: np.ndarray) -> Tuple[torch.Tensor, float, Tuple[int, int]]:
	...
	def _postprocess(
		...
		for i in range(len(boxes)):
			...
			cls_id = classes[i]
			name = str(result.names.get(cls_id, cls_id)) if isinstance(result.names, dict) else str(cls_id)
