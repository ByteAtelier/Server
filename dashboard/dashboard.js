...
function setupDashboard(
  io,
  { ingestBus, videoBus, options = {}, params = {}, statusProviders = {} } = {},
) {
  ...
  function buildSnapshot() {
    ...
    const cpuPercent = Number(
      ((cpuDeltaUs / 1000 / (cpuElapsedMs * cpuCoreCount)) * 100).toFixed(2),
    );
    ...
    const ingestBusStatus =
      typeof ingestBus?.getStatus === "function" ? ingestBus.getStatus() : {};
    const videoBusStatus =
      typeof videoBus?.getStatus === "function" ? videoBus.getStatus() : {};
    const modules = {
      ingestBus: {
        ...ingestBusStatus,
        alive: Boolean(
          ingestLastSeenAt && now - ingestLastSeenAt <= moduleAliveMs,
        ),
      ...
      videoBus: {
        ...videoBusStatus,
        alive: Boolean(
          videoLastSeenAt && now - videoLastSeenAt <= moduleAliveMs,
        ),
